...
app.post('/api/investigations', async (req, res, next) => {
  try {
    ...
    const sampleDataText = JSON.stringify(result.metricSummaries.map((s: any) => ({ dataset: s.dataset, sampleRows: s.sampleRows })));

    // 1. Attempt deep Python multi-agent workflow (Planner -> Coding Agent -> DuckDuckGo Research -> Critic -> Compiler)
    let agentResult: any = null;
    const firstDataset = result.dataSources?.[0];
    if (firstDataset) {
      const datasetObj = await getDataset(firstDataset.id);
          ...
          child.on('close', (code) => {
            if (code === 0) {
              try {
                ...
                if (first >= 0) {
                  const parsed = JSON.parse(stdout.slice(first));
                  if (parsed.ok) return resolve(parsed);
                }
              } catch (e) {
                console.warn('Agent bridge parse error:', e);
              }
            }
            resolve(null);
          });
          child.on('error', () => resolve(null));
          child.stdin.write(JSON.stringify({ question, dataPath, workspace }));
          child.stdin.end();
          setTimeout(() => { child.kill(); resolve(null); }, 35000);
        });
      }
    }
    ...
    const marketSources = agentResult?.webSources || reasoning?.sources || [];
    const marketAvailable = Boolean(marketSources.length);
    const ai = agentResult?.ai || (reasoning ? { provider: reasoning.provider, model: reasoning.model, response: reasoning.text, parsed, sources: reasoning.sources, grounded: Boolean(parsed && result.findings.length > 0) } : { provider: null, model: null, response: null, parsed: null, sources: [], grounded: false });
    const enriched = {
      ...result, ai,
      recommendation: parsed?.recommendation || result.recommendation,
      confidence: parsed?.confidence === null || parsed?.confidence === undefined ? result.confidence : Math.max(result.confidence, Math.min(95, parsed.confidence)),
      marketStatus: marketAvailable ? 'available' : (result.questionSignals?.asksMarket ? 'MARKET INTELLIGENCE UNAVAILABLE' : result.marketStatus),
      audit: {
        ...result.audit,
        llmUsed: Boolean(agentResult?.ai || reasoning),
        llmProvider: agentResult?.ai?.provider || reasoning?.provider || null,
        llmStructured: Boolean(parsed),
        externalSources: marketSources.length,
        evidenceCoverage: result.evidenceCoverage,
        dataQualityScore: result.dataQualityScore,
        toolTrace: [
          'dataset_profiler',
          'metric_calculator',
          ...(agentResult ? ['planner_agent', 'coding_agent', 'duckduckgo_search', 'critic_agent'] : []),
          'compiler'
        ]
      },
    };
    await updateInvestigation(result.runId, { ai: enriched.ai, recommendation: enriched.recommendation, confidence: enriched.confidence, marketStatus: enriched.marketStatus, audit: enriched.audit, findings: enriched.findings });
