...
app.post('/api/investigations', async (req, res, next) => {
  try {
    ...
    await updateInvestigation(result.runId, { ai: enriched.ai, audit: enriched.audit });
...
app.post('/api/ask', async (req, res, next) => { try { const { workspaceId, question, runId } = req.body ?? {}; if (!workspaceId || !question?.trim()) return res.status(400).json({ error: 'workspaceId and question are required' }); const investigation = runId ? await getInvestigation(runId) : (await listInvestigations(workspaceId))[0]; if (!investigation) return res.status(404).json({ error: 'No investigation exists for this workspace yet' }); const response = await askLLM({ system: `You are CorporateBaddie answering a follow-up question about an existing business investigation. Only use the evidence contained in the investigation below. Never invent facts. If the evidence does not answer the question, say that clearly.\n\nInvestigation:\n${JSON.stringify(investigation, null, 2)}`, prompt: question.trim(), useWebSearch: false }); res.json({ answer: response.text, provider: response.provider, model: response.model, sources: response.sources, runId: investigation.runId }); } catch (error) { next(error); } });
app.use((error: unknown, _req: express.Request, res: express.Response, _next: express.NextFunction) => { const message = error instanceof Error ? error.message : 'Internal server error'; const status = message.includes('Only CSV') || message.includes('File too large') ? 400 : 500; res.status(status).json({ error: message }); });
