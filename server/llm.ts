...
async function callGrok(request: LLMRequest): Promise<LLMResult> {
  ...
  const tools = request.useWebSearch
    ? [{ type: 'web_search' }]
    : undefined;
  ...
  return {
    provider: 'grok',
    model: data?.model || process.env.XAI_MODEL || 'grok-4.6',
    text,
    sources: extractGrokSources(data),
  };
}
...
export async function askLLM(request: LLMRequest): Promise<LLMResult> {
  const primary = (process.env.LLM_PRIMARY || 'grok') as Provider;
