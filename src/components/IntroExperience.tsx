...
export interface IntroExperienceProps {
  onEnter: (initialQuestion?: string) => void;
}

type NodeKey = 'DATA' | 'ANALYSIS' | 'MARKET' | 'EVIDENCE' | 'RISK' | 'DECISION';

const NODES: Array<{ key: NodeKey; detail: string; description: string; color: number; shortcut: string }> = [
  { key: 'DATA', detail: 'Your business data', description: 'Internal records, financial ledgers, CRM cohorts, and datasets.', color: 0x9caf9e, shortcut: '1' },
  { key: 'ANALYSIS', detail: 'Pattern recognition', description: 'Statistical analysis, anomaly detection, and empirical metrics.', color: 0xaab2aa, shortcut: '2' },
  { key: 'MARKET', detail: "External market context", description: 'Competitor benchmarks, industry trends, and pricing intelligence.', color: 0xaab2aa, shortcut: '3' },
  { key: 'EVIDENCE', detail: 'Evidence verification', description: 'Verified claims backed by immutable hashes and dataset records.', color: 0x9caf9e, shortcut: '4' },
  { key: 'RISK', detail: 'Failure modes & blind spots', description: 'Downside risks, stress testing, and counterfactual simulation.', color: 0xc18a80, shortcut: '5' },
  { key: 'DECISION', detail: 'Defensible recommendation', description: 'Evidence-backed strategy and decision governance options.', color: 0x9caf9e, shortcut: '6' },
];
...
const ORBITS = [
  ...
  { radius: 5.0, yScale: 0.82, tiltX: -0.52, tiltY: 0.2, tiltZ: 0.18, speed: -0.035 },
...
export default function IntroExperience({ onEnter }: IntroExperienceProps) {
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        skip();
      } else if (e.key === 'Enter' && !exiting) {
        start();
      } else {
        const num = parseInt(e.key, 10);
        if (num >= 1 && num <= NODES.length) {
          setActive(NODES[num - 1].key);
        }
      }
    };
  ...
  useEffect(() => {
    ...
    const width = Math.max(1, mount.clientWidth || window.innerWidth * 0.55);
    const height = Math.max(1, mount.clientHeight || window.innerHeight * 0.7);
    ...
    const key = new THREE.DirectionalLight(0xf1f3ef, 1.15);
    key.position.set(5, 6, 8);
    scene.add(key);
    const fill = new THREE.DirectionalLight(0x8eaa98, 0.35);
    ...
    const outer = new THREE.Mesh(
      new THREE.IcosahedronGeometry(3.25, 3),
      new THREE.MeshBasicMaterial({ color: 0xa6b3aa, wireframe: true, transparent: true, opacity: 0.34, depthWrite: false, depthTest: false })
    );
    sphere.add(outer);
    const inner = new THREE.Mesh(
      new THREE.IcosahedronGeometry(2.75, 2),
      new THREE.MeshBasicMaterial({ color: 0xc2cbc4, wireframe: true, transparent: true, opacity: 0.22, depthWrite: false })
    );
    sphere.add(inner);
    ...
    for (let i = 0; i < 15; i += 1) {
      const barHeight = 0.8 + ((i * 17) % 9) * 0.22;
      const bar = new THREE.Mesh(
        new THREE.BoxGeometry(0.11, barHeight, 0.11),
        new THREE.MeshBasicMaterial({ color: 0xd1d8d1, transparent: true, opacity: 0.26, depthWrite: false })
      );
      bar.position.set((i - 7) * 0.22, -0.35 + barHeight / 2, 0.15 * Math.sin(i));
      bars.add(bar);
    }
    sphere.add(bars);

    const orbitLines = ORBITS.map((config, index) => {
      const line = createOrbit(config, index === 4 ? 0xc18a80 : 0x9caf9e);
      root.add(line);
      return line;
    });
    ...
    const projectLabels = () => {
      ...
      nodeGroups.forEach((item, index) => {
        const p = item.group.position.clone().project(camera);
        next[NODES[index].key] = {
          x: (p.x * 0.5 + 0.5) * w,
          y: (-p.y * 0.5 + 0.5) * h,
          visible: p.z < 1 && Math.abs(p.x) < 1.2 && Math.abs(p.y) < 1.15,
        };
      });
      setLabels(next);
    };
    ...
    const animate = () => {
      ...
      if (!converging && !reduced) {
        sphere.rotation.y += delta * 0.055;
        sphere.rotation.x = Math.sin(time * 0.18) * 0.035;
        root.rotation.y = Math.sin(time * 0.12) * 0.035;
        orbitLines.forEach((line, i) => {
          line.rotation.z += delta * ORBITS[i].speed;
        });
      }
      ...
      nodeGroups.forEach((item, index) => {
        ...
        const t = time * config.speed + index * ((Math.PI * 2) / NODES.length);
        const target = new THREE.Vector3(
          Math.cos(t) * config.radius,
          Math.sin(t * 0.9 + index) * 1.2,
    ...
    const startConvergence = (targetQuestion?: string) => {
    ...
    <main className={`cb-intro-v2 ${exiting ? 'is-exiting' : ''}`} aria-label="CorporateBaddie introduction">
      <header className="cb-intro-v2__header">
        <div className="cb-intro-v2__brand">
          ...
          <span className="cb-intro-v2__divider">|</span>
          <span>INTRO</span>
        </div>
        <div className="cb-intro-v2__section">DECISION INTELLIGENCE / 01</div>
        <button className="cb-intro-v2__skip" type="button" onClick={skip}>
      ...
      <section className="cb-intro-v2__body">
        <div className="cb-intro-v2__copy">
          ...
          <p>
            Investigate your data, verify facts with deterministic logic, check external market signals, and test assumptions before taking action.
          </p>
          <div className="cb-intro-v2__example-label">EXAMPLE QUESTION</div>
          <button
            className="cb-intro-v2__question"
            type="button"
            onClick={() => start('Why are our margins falling?')}
            aria-label="Investigate example question: Why are our margins falling?"
          >
            <span>Why are our margins falling?</span>
          ...
          {NODES.map((node) => {
            ...
            return position ? (
              <button
                key={node.key}
                type="button"
                className={`cb-intro-v2__node-label ${selected ? 'is-active' : ''}`}
                style={{ left: position.x, top: position.y, opacity: position.visible ? 1 : 0 }}
                onClick={() => setActive(node.key)}
                aria-label={`${node.key}: ${node.description}`}
              >
                <span>{node.key}</span>
                {selected && <small>{node.detail}</small>}
              </button>
            ) : null;
          })}
          {active && (
            <div className="cb-intro-v2__node-info">
              ...
              <p>{NODES.find((node) => node.key === active)?.description}</p>
            </div>
          )}
        </div>
      </section>
        ...
        <div>
          ...
          <div className="cb-intro-v2__steps">
            {NODES.map((node, index) => (
              <span
                key={node.key}
                className={active === node.key ? 'is-current' : ''}
                onClick={() => setActive(node.key)}
                style={{ cursor: 'pointer' }}
                title={`Press ${node.shortcut} to view`}
              >
                {String(index + 1).padStart(2, '0')} {node.key}
              </span>
            ))}
          </div>
        </div>
        <div className="cb-intro-v2__principle">— EVIDENCE OVER ELOQUENCE.</div>
      </footer>
      ...
      <style>{`
        ...
        .cb-intro-v2__header {
          height: 66px;
          border-bottom: 1px solid rgba(196, 205, 196, 0.1);
          display: flex;
          align-items: center;
        ...
        .cb-intro-v2__dot {
          ...
          border-radius: 50%;
          background: #91aa95;
          box-shadow: 0 0 0 3px rgba(145, 170, 149, 0.08);
        }
        .cb-intro-v2__divider {
          opacity: 0.3;
        }
        .cb-intro-v2__section {
          position: absolute;
          left: 50%;
        ...
        .cb-intro-v2__skip:hover {
          color: #d4ddd5;
        }
        .cb-intro-v2__skip span {
          font-size: 15px;
          margin-left: 7px;
        }
        .cb-intro-v2__body {
          ...
          min-height: 520px;
          display: grid;
          grid-template-columns: minmax(380px, 0.82fr) minmax(620px, 1.35fr);
          gap: 1vw;
          align-items: center;
          padding: 0 6vw;
          position: relative;
        ...
        .cb-intro-v2__kicker,
        .cb-intro-v2__example-label,
        .cb-intro-v2__footer-kicker {
          font: 600 10px 'JetBrains Mono', monospace;
        ...
        .cb-intro-v2__question {
          width: min(410px, 100%);
          display: flex;
          justify-content: space-between;
          align-items: center;
          border: 1px solid rgba(196, 205, 196, 0.14);
        ...
        .cb-intro-v2__question:hover {
          border-color: rgba(156, 175, 158, 0.4);
          background: rgba(22, 28, 25, 0.85);
          transform: translateY(-1px);
        }
        .cb-intro-v2__question-arrow {
          color: #9caf9e;
          font-size: 18px;
        }
        .cb-intro-v2__actions {
          display: flex;
          gap: 26px;
          align-items: center;
          margin-top: 22px;
        ...
        .cb-intro-v2__enter:hover {
          background: #a3bda5;
          transform: translateY(-1px);
          box-shadow: 0 4px 14px rgba(145, 169, 147, 0.25);
        }
        .cb-intro-v2__secondary {
          border: 0;
          background: none;
          color: #7e8a82;
          font: 600 10px 'JetBrains Mono', monospace;
        ...
        .cb-intro-v2__canvas canvas {
          width: 100%;
          height: 100%;
          display: block;
        }
        .cb-intro-v2__node-label {
          position: absolute;
          ...
          font: 600 10px 'JetBrains Mono', monospace;
          letter-spacing: 0.12em;
          text-shadow: 0 1px 10px #0a0d0c;
          transition: color 0.16s ease, opacity 0.2s ease, transform 0.15s ease;
        }
        .cb-intro-v2__node-label:hover,
        .cb-intro-v2__node-label.is-active {
          color: #b6ccb9;
          transform: translate(-50%, -50%) scale(1.08);
        }
        .cb-intro-v2__node-label small {
          display: block;
          margin-top: 5px;
          font-size: 9px;
          letter-spacing: 0.04em;
        ...
        .cb-intro-v2__node-info {
          position: absolute;
          right: 2%;
          bottom: 9%;
          width: 230px;
          border-left: 1px solid rgba(142, 179, 151, 0.45);
        ...
        .cb-intro-v2__node-info span {
          font: 600 9px 'JetBrains Mono', monospace;
          letter-spacing: 0.15em;
          color: #91aa95;
        }
        .cb-intro-v2__node-info strong {
          display: block;
          font-size: 12px;
          margin-top: 5px;
          font-weight: 600;
        ...
        .cb-intro-v2__footer {
          ...
          right: 0;
          height: 90px;
          border-top: 1px solid rgba(196, 205, 196, 0.09);
          display: flex;
          justify-content: space-between;
          align-items: center;
          padding: 0 6vw;
        ...
        .cb-intro-v2__steps span:hover {
          color: #a4b4a6;
        }
        .cb-intro-v2__steps span.is-current {
          color: #d4ddd5;
          text-decoration: underline;
          text-underline-offset: 4px;
        }
        .cb-intro-v2__principle {
          font: 600 9px 'JetBrains Mono', monospace;
          letter-spacing: 0.13em;
          color: #67736b;
        }
