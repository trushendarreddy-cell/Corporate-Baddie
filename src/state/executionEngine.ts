...
import { workspaceRepo } from './workspaceRepository';
...
const issue = (status: InvestigationIssue['status'], failedComponent: string, impact: string, nextAction: string): InvestigationIssue => ({
  status,
  failedComponent,
  impact,
  confidenceDelta: status === 'DATA INSUFFICIENT' ? -30 : -10,
  nextAction,
});

const backendStatus = (result: ApiInvestigation): StageRunStatus => {
  if (result.status === 'data_insufficient') return 'DATA INSUFFICIENT';
  return 'COMPLETED';
};
...
export const executeInvestigation = async (options: ExecuteInvestigationOptions): Promise<ExecutionOutcome> => {
  ...
  const datasetIds = options.dataSources.filter((s) => s.selected && s.recordsCount > 0).map((s) => s.id);
  ...
  const emit = async (stage: StageId, status: StageRunStatus, summary: string, outputKeys: string[], canContinue: boolean) => {
    ...
    const result: StageResult = {
      ...
      executorId: meta[2],
      executorName: meta[3],
      executorCategory: ['REQUIREMENT_ANALYSIS', 'ROOT_CAUSE', 'MARKET_INTELLIGENCE', 'EVIDENCE_VERIFICATION', 'RECOMMENDATION'].includes(stage)
        ? 'REASONING_AGENT'
        : 'ANALYTICAL_SERVICE',
    };
    ...
    logs.push(entry);
    results.push(result);
    await options.onStage?.(result, entry);
  };

  let backend: ApiInvestigation;
  try {
    ...
    await emit('DATA_PROFILER', datasetIds.length ? 'COMPLETED' : 'DATA INSUFFICIENT', datasetIds.length ? 'Selected datasets will be profiled from persisted server-side records.' : 'No selected dataset contains usable records.', ['dataSources'], datasetIds.length > 0);
    if (!datasetIds.length) {
      const blocked = issue('DATA INSUFFICIENT', 'Data intake', 'No persisted dataset can support an evidence-backed investigation.', 'Upload and select at least one CSV or Excel dataset.');
    ...
    backend = await api.investigate(workspaceId, options.question, datasetIds);
    ...
    const terminalStatus: 'COMPLETED' | StageRunStatus = backend.status === 'data_insufficient' ? 'DATA INSUFFICIENT' : 'COMPLETED';
    return {
      runId: backend.runId,
      results,
      log: logs,
      state: { backend },
      terminalStatus,
      issues: terminalStatus === 'COMPLETED' ? [] : [issue('DATA INSUFFICIENT', 'Backend investigation', 'The server could not establish usable evidence.', 'Upload a usable dataset and rerun the investigation.')],
      backend,
    };
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Backend investigation failed.';
    const failure = issue('TOOL FAILURE', 'Investigation API', message, 'Check that the API server is running on port 4000, then rerun.');
    await emit('RECOMMENDATION', 'TOOL FAILED', message, [], false);
    return { runId: options.runId, results, log: logs, state: {}, terminalStatus: 'TOOL FAILED', issues: [failure], backend: { runId: options.runId, workspaceId, question: options.question, status: 'tool_failure', dataSources: [], findings: [], metricSummaries: [], marketStatus: 'TOOL FAILED', forecastStatus: 'TOOL FAILED', recommendation: 'TOOL FAILED: no recommendation issued.', confidence: 0, audit: { backend: false, error: message } } };
  }
};
...
export const applyExecutionOutcomeToState = (base: UnifiedInvestigationState, outcome: ExecutionOutcome): UnifiedInvestigationState => {
  ...
  const stagePlan = base.investigationPlan.map((node) => ({ ...node, status: node.status }));
  const auditEvents = [
    ...(base.auditEvents || []),
    {
      id: `backend-${backend.runId}`,
      timestamp: new Date().toISOString(),
      type: 'INVESTIGATION_EXECUTED',
      actor: 'CorporateBaddie backend',
      description: `Backend run ${backend.runId} used persisted dataset rows and produced ${verified.length} verified fact(s).`,
      metadata: backend.audit,
    } as any,
  ];
  ...
  return {
    ...
    timestamp: new Date().toISOString(),
    activeDataSources: base.activeDataSources,
    investigationPlan: stagePlan,
    claims,
    empiricalFindings,
    discoveredAnomalies: [],
    marketIntelligence: [],
    ...
    recommendationExplanation: verified.length
      ? 'Recommendation is grounded in findings calculated from persisted dataset rows. No unsupported causal claim is added.'
    ...
    recommendationConfidence: {
      ...
      topBoosters: verified.length ? ['Persisted rows were used for deterministic calculations.', 'Findings are linked to dataset sources.'] : [],
      topReducers: backend.marketStatus === 'MARKET INTELLIGENCE UNAVAILABLE' ? ['External market context was unavailable.'] : [],
...
export const deriveDisplayStages = (results: StageResult[]): InvestigationStage[] => results.map((result, index) => ({
  id: index + 1,
  title: stageMeta.find(([id]) => id === result.stage)?.[1] || result.stage,
  status: result.status === 'COMPLETED' ? 'completed' : result.status === 'SKIPPED' ? 'pending' : 'pending',
  dynamicMessage: result.summary,
  explanation: result.summary,
  logMessage: result.summary,
  durationMs: result.durationMs,
  findingsCount: result.outputKeys.length,
  executorName: result.executorName,
  executorCategory: result.executorCategory,
}));
